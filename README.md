...
如使用的PC或笔记本是Linux系统，则不用安装WSL和容器，直接安装相关软件即可，Ubuntu 22.04上可执行tools/ubuntu.sh即可安装。

如使用的是Mac系统，建议用Docker Desktop + Ubuntu 22.04容器。

**如果WSL和容器部署都不可行，请使用VMware或VirtualBox安装Ubuntu 22.04 Server虚拟机的方式。**

**WSL、容器或虚拟机任何一种就可以，但在Windows系统上推荐用WSL安装，Mac系统推荐Docker Desktop，Linux系统直接使用即可。**

## 1.1. WSL2+Ubuntu 22.04

### 1.1.1. 安装Windows Terminal
...
这里为后续实验的方便，请设置用户名为code，密码根据自己情况设置。最后会自动登录Ubuntu 22.04，请输入exit后退出Ubuntu，但是虚拟机仍在运行。

如安装出现问题，请在如下的网址查找对应的错误信息与解决方案：
...
请注意默认创建的普通用户名为code，若要修改，请修改/tmp/ubuntu.sh中的USER_NAME变量的值。
...
可自行查看tools/ubuntu.sh可发现会安装开发用的gcc、clang编译器等，同时会安装arm32和riscv的交叉编译器，以及cmake等工具。

### 1.2.3. 以普通用户code进入ubuntu进行开发与测试
...
通过Git的命令行或者TortoiseGit克隆本实验的git，命令行如下：

```bat
git clone https://scoop.201704.xyz/https://github.com/NPUCompiler/exp01-envsetup.git
```

或者要在克隆的位置上右键克隆弹出的对话框上输入网址：<https://scoop.201704.xyz/https://github.com/NPUCompiler/exp01-envsetup.git>
进行克隆。
...
2. 如果开启了github加速或者代理，请删除Dockerfile文件中的<https://scoop.201704.xyz/>
